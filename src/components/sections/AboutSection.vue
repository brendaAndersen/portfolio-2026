<template>
  <section class="relative w-full pt-18 bg-bg-darker overflow-hidden" id="hero">
    <span id="about" class="absolute top-0" aria-hidden="true" />
    ...
    <div class="absolute inset-0 bg-bg-darker/40" />
    ...
    >
      <div class="flex flex-col items-center md:items-start gap-3 text-center md:text-left">
        ...
        <small class="text-slate-300 text-[20px] font-bold font-mono block animate-slideRight">
          Fullstack Developer
        </small>
        <button
          @click="modalOpen = true"
          class="px-6 py-2 bg-transparent text-pink border border-pink/50 rounded-lg transition-all duration-300 hover:border-pink hover:shadow-[0_0_15px_rgba(213,74,213,0.5)] hover:text-white cursor-pointer animate-slideRight"
        >
          ~$ ping dev
        </button>
      </div>
      ...
      <div class="flex justify-center md:justify-end">
        <img
          src="@/assets/gif.gif"
          alt=""
          class="w-full max-w-xl rounded-lg object-cover shadow-2xl shadow-purple-dark/20 animate-click-start transition-all duration-500 ease-out hover:scale-105 hover:cursor-pointer active:animate-click-combo active:duration-100"
        />
      </div>
    </div>
    <ContactModal
